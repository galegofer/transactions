...
public class TransactionsService {
    ...
    private Function<Object, BBResponseDTO> typeConverter;
    ...
    public List<BBResponseDTO> getTransactionsList() {
        ...
        if (status.is2xxSuccessful()) {
            ...
            Optional<OpenBankResponseDTO> transactions = Optional.ofNullable(body);

            if (transactions.isPresent()) {
                return transactions.get().getTransactions().stream().map(typeConverter).collect(toList());
            }
        }
    ...
    public List<BBResponseDTO> getTransactionsPerType(String transactionType) {
        ...
        if (status.is2xxSuccessful()) {
            ...
            if (transactions.isPresent()) {
                return transactions.get().getTransactions().stream().filter(transactionDTO -> {
    ...
    public Double getTransactionTotalsPerType(String transactionType) {
        ...
        if (status.is2xxSuccessful()) {
            ...
            if (transactions.isPresent()) {
                return transactions.get().getTransactions().stream().filter(transactionDTO -> {
                    return transactionType.equals(JsonPath.read(transactionDTO, "$.details.type"));
                }).map(typeConverter).map(BBResponseDTO::getTransactionAmount).reduce(0d,
